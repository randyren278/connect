 ...
 *  MASTER “SEEDS” ARRAYS                                              *
 * ------------------------------------------------------------------ *
 *  These arrays list every human‑readable theme title the game can    *
 *  present.  They map 1‑to‑1 (after normalisation) to keys inside     *
 *  CURATED_SETS.  No network calls are ever made; only these themes   *
 *  and their word lists are used.                                     *
 * ------------------------------------------------------------------ */
 ...
 *  CURATED WORD LISTS                                                *
 * ------------------------------------------------------------------ *
 *  Keys appear in ALL‑CAPS, matching the normalised form of the       *
 *  titles above.  Each array has ≥ 4 entries; getSynonyms shuffles     *
 *  then returns the first four.  (Only a subset shown here for        *
 *  brevity, but keep all you listed.)                                 *
 */
 ...
 *  MAIN: getSynonyms                                                 *
 * ------------------------------------------------------------------ *
 *  Purely local — picks 4 words from CURATED_SETS, never hitting the  *
 *  network.  If a theme isn't found, returns a placeholder quartet.   *
 * ------------------------------------------------------------------ */
export async function getSynonyms(keyword: string): Promise<string[]> {
  const key = normalize(keyword);
  if (key in CURATED_SETS) {
    return shuffleArray(CURATED_SETS[key]).slice(0, 4);
  }
  return [`NO`, `WORDS`, `FOR`, key].slice(0, 4);
}
